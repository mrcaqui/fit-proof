...
-- Submission Rules table (hierarchical: Month > Week > Day)
create table submission_rules (
  id bigint generated by default as identity primary key,
  client_id uuid references profiles(id) on delete cascade not null,
  rule_type text check (rule_type in ('deadline', 'target_day')) not null,
  ...
  specific_date date,
  value text, -- time "19:00" for deadline, "true"/"false" for target_day, or null to clear override
  created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
-- Clients can view their own rules
